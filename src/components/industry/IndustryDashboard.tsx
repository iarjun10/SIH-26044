...
import type { Internship, ApplicationWithDetails, StudentSkills, Profile, ApplicationStatus } from '@/types';
import { Plus, Building2, Users, TrendingUp, X, Loader2, MapPin, Clock, IndianRupee } from 'lucide-react';
        ...
        const { data: appsData } = await supabase
          ...
          .select(`
            *,
            ...
            student_skills:student_skills!applications_student_id_fkey(*)
          `)
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white font-medium rounded-lg transition-colors text-sm"
        >
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Internships Posted</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-5">
      ...
      {/* Internship list with applicants */}
      {internships.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
          <Building2 className="w-12 h-12 text-slate-300 mx-auto mb-4" />
          <h2 className="text-lg font-semibold text-slate-900 mb-2">No Internships Posted</h2>
          <p className="text-sm text-slate-500 mb-6">Post your first internship to start receiving applications.</p>
          <button onClick={() => setShowModal(true)} className="px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white font-medium rounded-lg text-sm">
            Post Internship
          </button>
        </div>
      ) : (
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                !selectedInternship ? 'bg-blue-50 text-blue-700' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
              }`}
            >
            ...
            {internships.map((intern) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                    selectedInternship === intern.id ? 'bg-blue-50 text-blue-700' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                  }`}
                >
                  {intern.title} ({count})
                </button>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {selectedApps.map((app) => (
                      <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <p className="text-sm text-slate-700">{app.internship?.title}</p>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`text-sm font-bold px-2.5 py-1 rounded-full ${
                            app.match_score >= 70 ? 'bg-green-50 text-green-700' :
                            app.match_score >= 40 ? 'bg-amber-50 text-amber-700' :
                        ...
                        <td className="px-6 py-4">
                          <select
                            ...
                            className={`text-xs font-medium px-3 py-1.5 rounded-lg border cursor-pointer outline-none transition-all ${
            ...
            <form onSubmit={handleCreateInternship} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} required rows={3}
                  className="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all text-sm resize-none"
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  <input type="text" value={skillInput} onChange={(e) => setSkillInput(e.target.value)}
                    ...
                    className="flex-1 px-4 py-2.5 rounded-lg border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all text-sm"
                ...
                <div className="flex flex-wrap gap-1.5">
                  {requiredSkills.map((skill) => (
                    <span key={skill} className="flex items-center gap-1 text-xs px-2.5 py-1 bg-blue-50 text-blue-700 rounded-md">
                      {skill}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={submitting}
                  className="flex-1 py-2.5 bg-blue-700 hover:bg-blue-800 text-white font-medium rounded-lg text-sm transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                  {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Post Internship'}
                </button>
              </div>
            </form>
          </div>
